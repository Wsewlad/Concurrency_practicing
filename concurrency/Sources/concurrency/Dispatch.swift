...
func dispatchStorageAndCoordination() {
    func makeDatabaseQuery() {
        ...
        Thread.sleep(forTimeInterval: 0.5)
        print(requestId, "Finishing database query")
    }
    ...
    func makeNetworkRequest() {
        ...
        Thread.sleep(forTimeInterval: 0.5)
        print(requestId, "Finishing network request")
    }
    ...
    func response(for request: URLRequest, queue: DispatchQueue) -> HTTPURLResponse {
        ...
        let databaseQueryQueue = DispatchQueue(label: "database-query", target: queue)
        databaseQueryQueue.async(group: group) {
            makeDatabaseQuery()
        }
        
        let networkRequestQueue = DispatchQueue(label: "network-request", target: queue)
        networkRequestQueue.async(group: group) {
            makeNetworkRequest()
        }
    ...
    queue1.setSpecific(key: idKey, value: 24)
